...
class Query(t.Generic[T]):
    ...
    def create(self, **kwargs: dict[str, t.Any]) -> T:
        ...
        print("TODO: fix")
        row = query_one(f"SELECT * FROM {self.model()._get_tablename()} WHERE id = ?", (last_id,))
    ...
    def latest(self, date_field: str | Date | Field[datetime] | None = None) -> T | None:
        ...
        if result:
            return self.model._from_db(result)
